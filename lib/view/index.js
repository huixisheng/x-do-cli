...
function init(ViewsPath) {
  ...
  directory.forEach((value, key) => {
    ...
    debuglog('subDirectory', subDirectory);
    // @todo 分类目录下为空
    ...
    let result = subDirectory.map((vfile) => {
      ...
      // 驼峰
      let routerName = value.replace('/', '') + plusString.capitalize(plusString.camelize(filename));
      ...
      try {
        ...
        // debuglog(vueScriptObject.data);
        // console.log(vueScriptObject.title);
        ...
        console.log(parseVueScriptInstance.parseVueFile());
        console.error(absoluteComponentPath, ' parse error');
        console.error(error);
      }
    ...
    list.forEach((value) => {
      ...
      debuglog('生成文件%s成功', viewRouterChildrenPath);

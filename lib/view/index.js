...
function init(ViewsPath) {
  ...
  directory.forEach((value, key) => {
    ...
    const routerPath = plusString.dasherize(value.replace('/', ''));

    let result = subDirectory.map((vfile) => {
      const filename = path.basename(vfile, '.vue');
      const name = filename.toLowerCase();
      const componentName = plusString.classify(routerPath) + plusString.classify(name);
      ...
      const camelizeComponentName = plusString.camelize(componentName);

      const fileContent = fs.readFileSync(absoluteComponentPath, 'utf-8');
      const title = fileContent.match(/title:\s?['"](.+)['"]/)[1];
      ...
      return {
        ...
        componentName,
        componentPath,
        componentNameCate: plusString.classify(routerPath),
        componentFileCate: plusString.dasherize(routerPath),
        camelizeComponentName,
        title,
        hidden,
      };
